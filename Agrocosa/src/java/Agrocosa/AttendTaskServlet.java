...
public class AttendTaskServlet extends SIDServlet {

    String taskTxn = "AgrocosaTransactions.RetrieveTaskToAttendTransaction";
    String updateTaskTxn = "AgrocosaTransactions.UpdateTaskStatusTransaction";
    String tractorTxn = "AgrocosaTransactions.TractorMenuTransaction";
    String toolTxn = "AgrocosaTransactions.ToolMenuTransaction";
    String bombSupervisorTxn = "AgrocosaTransactions.BombSupervisorMenuTransaction";
    ...
    protected void processRequest(HttpServletRequest request,
        ...
        try {
            ...
            session.setAttribute("acordion", "TaskUser");
            ...
            if (nodeArray != null) {
                if (nodeArray.getSize() > 0
                        ...
                        && nodeArray.find("Action").getStringValue().equals("AttendTask")) {
                    callTxnNA.find("TRANSACTION_CLASS_TO_EXECUTE").setValue(updateTaskTxn);
            ...
            callTxnNA.find("TRANSACTION_CLASS_TO_EXECUTE").setValue(bombSupervisorTxn);
            txnNA = executeTransaction(callTxnNA);
            if (txnNA != null) {
                txnNA.remove("RESPONSE_CODE");
                txnNA.remove("RESPONSE_MESSAGE");
            ...
            if (redirectNA != null) {
                redirectNA.add("JspPage", "/attendTask.page");
                session.setAttribute("TxData", redirectNA);
                response.sendRedirect("management.do");
            } else {
                rd = request.getRequestDispatcher("/attendTask.page");
