...
public class WarehouseProductManagementServlet extends SIDServlet {
    String pruductTypeNameTxn = "AgrocosaTransactions.RetrieveWarehouseTypeNameTransaction";
    String managementTxn = "AgrocosaTransactions.WarehouseProductManagementTransaction";
    ...
    protected void processRequest(HttpServletRequest request,
        ...
        try {
            ...
            callTxnNA.add("TRANSACTION_CLASS_TO_EXECUTE", pruductTypeNameTxn);
            ...
            if (nodeArray != null) {
                if (nodeArray.getSize() > 0
                    ...
                    callTxnNA.add("SEARCH","YES");
                    callTxnNA.find("TRANSACTION_CLASS_TO_EXECUTE").setValue(managementTxn);
                    xmlNodeArray tmp = executeTransaction(callTxnNA);
                    if (tmp != null) {
                        txnNA.remove("WarehouseProductx_Table");
                        txnNA.add("WarehouseProductx_Table", tmp.find("WarehouseProductx_Table").getTable());
                    }
                }
            }
            
            if (txnNA != null) {
                txnNA.add("JspPage", "/warehouseProductManagement.page");
                session.setAttribute("TxData", txnNA);
                response.sendRedirect("management.do");
            } else {
                rd = request.getRequestDispatcher("/warehouseProductManagement.page");
