...
public class EditCampServlet extends SIDServlet {
    ...
    String saveSectionTxn = "AgrocosaTransactions.SaveSelectedSectionTransaction";
    String deleteSectionTxn = "AgrocosaTransactions.DeleteCampSectionTransaction";
    ...
    protected void processRequest(HttpServletRequest request,
        ...
        try {
            ...
            callTxnNA.add("TRANSACTION_CLASS_TO_EXECUTE", landTypeTxn);            
            ...
            if (nodeArray != null) {
                if (nodeArray.getSize() > 0
                        ...
                        && nodeArray.find("Action").getStringValue().equals("SaveSection")) {
                    callTxnNA.find("TRANSACTION_CLASS_TO_EXECUTE").setValue(saveSectionTxn);
            ...
            if (nodeArray != null) {
                if (nodeArray.getSize() > 0
                        && nodeArray.exist("Action")
                        && !nodeArray.find("Action").isValueEmpty()
                        && nodeArray.find("Action").getStringValue().equals("DeleteCampSection")) {
                    callTxnNA.find("TRANSACTION_CLASS_TO_EXECUTE").setValue(deleteSectionTxn);
                    callTxnNA.add("idUser", session.getAttribute("idUser").toString());
            ...
            if (redirectNA != null) {
                redirectNA.add("JspPage", "/editCamp.page");
                session.setAttribute("TxData", redirectNA);
                response.sendRedirect("management.do");
            } else {
                rd = request.getRequestDispatcher("/editCamp.page");
