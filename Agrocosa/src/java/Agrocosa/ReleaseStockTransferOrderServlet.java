...
public class ReleaseStockTransferOrderServlet extends SIDServlet {
    String transportCompanyTxn = "AgrocosaTransactions.TransportCompanyMenuTransaction";
    //String cancelTxn = "AgrocosaTransactions.SaveCancelStockTransferOrderTransaction";
    String updateEmbarque = "AgrocosaTransactions.SaveUpdateStockTransferOrderTransaction";
    String releaseTxn = "AgrocosaTransactions.SaveReleaseStockTransferOrderTransaction";
    String transferOrderTxn = "AgrocosaTransactions.RetrieveTruckTransferOrderTransaction";
        ...
        try {
            ...
            if (nodeArray != null) {
                if (nodeArray.getSize() > 0
                        && nodeArray.exist("Action")
                        && !nodeArray.find("Action").isValueEmpty()
                        && nodeArray.find("Action").getStringValue().equals("UpdateOrder")) {
                    
                    callTxnNA.find("TRANSACTION_CLASS_TO_EXECUTE").setValue(updateEmbarque);
            ...
            if (nodeArray != null) {
                if (nodeArray.getSize() > 0
                    ...
                    callTxnNA.find("TRANSACTION_CLASS_TO_EXECUTE").setValue(releaseTxn);
            ...
            callTxnNA.add("status", "Nueva"); // Para que solo salgan Embarques que no se hayan asignado proforma
            if(callTxnNA.exist("proforma")){
                callTxnNA.find("proforma").setValue(null);
            }
            txnNA = executeTransaction(callTxnNA);
            if (txnNA != null) {
                txnNA.remove("RESPONSE_CODE");
            ...
            if (redirectNA != null) {
                redirectNA.add("JspPage", "/releaseStockTransferOrder.page");
                session.setAttribute("TxData", redirectNA);
                response.sendRedirect("management.do");
            } else {
                rd = request.getRequestDispatcher("/releaseStockTransferOrder.page");
