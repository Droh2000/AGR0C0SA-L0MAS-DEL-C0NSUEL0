...
<script type="text/javascript">
    ...
    function addComponent(idSprinkler, bomName)
    {
        $('#selectedIdBom').val(idSprinkler);
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            Administración de Aspersoras
            <small>Configuración de Aspersoras</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="sprinklerManagement.do"  onsubmit="">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Aspersoras</h3>
                </div>


                <div class="box-body" id="info" > 
                    <c:forEach items="${SprinklerInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idSprinkler}">
                                <div class="row">


                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <label>Nombre de Aspersora</label>                                                
                                            <div class="input-group">
                                                <input name="SprinklerName" type="text" class="form-control pull-right" id="SprinklerName" required value="${row.SprinklerName}" />
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                            <label>Boquilla</label>                                                
                                            <div class="input-group">
                                                <input name="Nozzle" type="text" class="form-control pull-right" id="Nozzle" required value="${row.Nozzle}" />
                                            </div>                                  
                                        </div>
                                ...
                                <input type="hidden" name="ID" title="${row.idSprinkler}" value="${row.idSprinkler}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.SprinklerInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'sprinklerManagement.do'">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <c:forEach items="${SprinklerInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idSprinkler and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idSprinkler}" value="${row.idSprinkler}"
                                                           ${requestScope.ID == row.idSprinkler and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
                                            <td>${row.SprinklerName}</td>
                                            <td>${row.Nozzle}</td>
                                            <td>${row.User}</td>
