...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
             Administracion de Variables del Sistema
            <small>Configuracion del catalogo de Variables del Sistema</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="configurationManagement.do"  onsubmit="">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Variables del Sistema</h3>
                </div>


                <div class="box-body" id="info" > 
                    <c:forEach items="${Configuration_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idConfig}">
                                <div class="row">
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <div class="col-sm-6">
                                                ...
                                                <div class="input-group">
                                                    <input name="Tag" type="text" class="form-control pull-right" id="ConfigurationName" required value="${row.Tag}" />
                                                </div>
                                            </div>
                                            <div class="col-sm-6">
                                                <label>Valor</label>                                                
                                                <div class="input-group">
                                                    <input name="Value" type="text" class="form-control pull-right" id="ConfigurationName" required value="${row.Value}" />
                                                </div>
                                            </div>
                                            <div class="col-sm-6">
                                ...
                                <input type="hidden" name="ID" title="${row.idConfig}" value="${row.idConfig}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.Configuration_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'configurationManagement.do'">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <c:forEach items="${Configuration_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idConfig and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idConfig}" value="${row.idConfig}"
                                                           ${requestScope.ID == row.idConfig and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
                                            <td>${row.Tag}</td>
                                            <td>${row.Value}</td>
