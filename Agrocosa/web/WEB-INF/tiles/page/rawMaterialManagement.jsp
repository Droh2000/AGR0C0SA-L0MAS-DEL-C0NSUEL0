...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            ...
            <small>Configuración de insumos</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="rawMaterialManagement.do"  onsubmit="">
            <div class="box box-success">
                ...
                <div class="box-body" id="info" > 
                    <c:forEach items="${RawMaterialInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idRawMaterial}">
                                <div class="row">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="RawMaterialTypeName" id="RawMaterialTypeName" class="form-control" required>
                                                ...
                                                <c:forEach items="${RawMaterialType_HashMap}" var="item">
                                                    <option value="${item.value}"  ${row.RawMaterialTypeName == item.value  ? 'selected' : ''}>${item.value}</option>
                                                </c:forEach>
                                            </select>  
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">   
                                                <select name="SupplierName" id="SupplierName" class="form-control" required>
                                                    <option value=""></option>
                                                    <c:forEach items="${Supplier_HashMap}" var="item">
                                                        <option value="${item.value}"  ${row.SupplierName == item.value  ? 'selected' : ''}>${item.value}</option>
                                                    </c:forEach>
                                                </select>  
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <div class="input-group"> 
                                                <select name="BrandName" id="BrandName" class="form-control" required>
                                                    <option value=""></option>
                                                    <c:forEach items="${Brand_HashMap}" var="item">
                                                        <option value="${item.value}"  ${row.BrandName == item.value  ? 'selected' : ''}>${item.value}</option>
                                                    </c:forEach>
                                                </select>  
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input name="RawMaterialName" type="text" class="form-control pull-right" id="RawMaterialName" required value="${row.RawMaterialName}" />
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                            <label >Unidad de Medida Base</label>
                                            <select name="DefaultUOM" id="DefaultUOM" class="form-control" data-title="Unida de Medida" required>
                                                <option value=""></option>
                                                <c:forEach items="${UOM_HashMap}" var="item">
                                                    <option value="${item.value}"  ${row.DefaultUOM == item.value  ? 'selected' : ''}>${item.value}</option>
                                                </c:forEach>
                                            </select>                                                                                     
                                        </div>
                                        <div class="form-group">
                                            <label>Costo</label>                                                
                                            <div class="input-group">
                                                <input name="Cost" type="text" class="form-control pull-right" id="Cost" required value="${row.Cost}" />
                                            </div>                                  
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                ...
                                <input type="hidden" name="ID" title="${row.idRawMaterial}" value="${row.idRawMaterial}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.RawMaterialInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'rawMaterialManagement.do'">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <c:forEach items="${RawMaterialInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idRawMaterial and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idRawMaterial}" value="${row.idRawMaterial}"
                                                           ${requestScope.ID == row.idRawMaterial and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
