...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
             Administración de Tipos de Labor
            <small>Configuración de tipos de labor</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="laborTypeManagement.do"  onsubmit="">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Tipos de Labor</h3>
                </div>


                <div class="box-body" id="info" > 
                    <c:forEach items="${LaborTypeInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idLaborType}">
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input name="LaborTypeName" type="text" class="form-control pull-right" id="LaborTypeName" required value="${row.LaborTypeName}" />
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                            <label></label>                                                
                                ...
                                <input type="hidden" name="ID" title="${row.idLaborType}" value="${row.idLaborType}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.LaborTypeInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'laborTypeManagement.do'">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <c:forEach items="${LaborTypeInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idLaborType and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idLaborType}" value="${row.idLaborType}"
                                                           ${requestScope.ID == row.idLaborType and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
