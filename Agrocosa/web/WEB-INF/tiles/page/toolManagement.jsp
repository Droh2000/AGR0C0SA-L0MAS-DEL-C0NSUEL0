...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            ...
            <small>Configuración de Herramientas</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="toolManagement.do"  onsubmit="">
            <div class="box box-success">
                ...
                <div class="box-body" id="info" > 
                    <c:forEach items="${ToolInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idTool}">
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        ...
                                        <div class="form-group">
                                            <label>Nombre</label>                                                
                                            <div class="input-group">
                                                <input name="ToolName" type="text" class="form-control pull-right" id="ToolName" required value="${row.ToolName}" />
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label>Descripcion</label>                                                
                                            <div class="input-group">  
                                                <input name="Description" type="text" class="form-control pull-right" id="Description" required value="${row.Description}" />
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                ...
                                <input type="hidden" name="ID" title="${row.idTool}" value="${row.idTool}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.ToolInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'toolManagement.do'">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <c:forEach items="${ToolInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idTool and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idTool}" value="${row.idTool}"
                                                           ${requestScope.ID == row.idTool and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
                                            <td>${row.Number}</td>
                                            <td>${row.ToolName}</td>
