...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
             Administración de impresoras para etiquetas
            <small>Configuración de impresoras</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="printerManagement.do" onsubmit="">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Definición de Impresoras</h3>
                </div>


                <div class="box-body" id="info" > 
                    <c:forEach items="${PrinterDefinitionInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idPrinterDefinition}">
                                <div class="row">
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <label>Nombre de la impresora</label>                                                
                                            <div class="input-group">
                                                <input name="PrinterName" type="text" class="form-control pull-right" id="PrinterName" required value="${row.PrinterName}" />
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                            <label></label>                                                
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <div class="input-group">
                                                ...
                                                <input type="text" name="User" id="User" class="form-control" value="${row.UserName}" readonly />
                                            </div>
                                        </div>
                                        <div class="form-group">
                                ...
                                <input type="hidden" name="ID" title="${row.idPrinterDefinition}" value="${row.idPrinterDefinition}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.PrinterDefinitionInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'printerManagement.do'">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <c:forEach items="${PrinterDefinitionInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idPrinterDefinition and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idPrinterDefinition}" value="${row.idPrinterDefinition}"
                                                           ${requestScope.ID == row.idPrinterDefinition and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
