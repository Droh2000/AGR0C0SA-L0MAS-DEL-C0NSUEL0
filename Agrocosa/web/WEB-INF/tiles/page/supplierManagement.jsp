...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            ...
            <small>Configuración de proveedores de insumos</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="supplierManagement.do"  onsubmit="">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Proveedores de insumos</h3>
                </div>


                <div class="box-body" id="info" > 
                    <c:forEach items="${SupplierInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idSupplier}">
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input name="SupplierName" type="text" class="form-control pull-right" id="SupplierName" required value="${row.SupplierName}" />
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                            <label></label>                                                
                                ...
                                <input type="hidden" name="ID" title="${row.idSupplier}" value="${row.idSupplier}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.SupplierInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'supplierManagement.do'">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <c:forEach items="${SupplierInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idSupplier and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idSupplier}" value="${row.idSupplier}"
                                                           ${requestScope.ID == row.idSupplier and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
