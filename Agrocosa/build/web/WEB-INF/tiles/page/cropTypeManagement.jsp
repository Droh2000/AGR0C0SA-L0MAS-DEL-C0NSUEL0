...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            ...
            <small>Configuración de cultivos</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="cropTypeManagement.do"  onsubmit="">
            <div class="box box-success">
                ...
                <div class="box-body" id="info" > 
                    <c:forEach items="${CropTypeInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idCropType}">
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input name="CropTypeName" type="text" class="form-control pull-right" id="CropTypeName" required value="${row.CropTypeName}" />
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                            <label></label>                                                
                                ...
                                <input type="hidden" name="ID" title="${row.idCropType}" value="${row.idCropType}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.CropTypeInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'cropTypeManagement.do'">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <c:forEach items="${CropTypeInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idCropType and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idCropType}" value="${row.idCropType}"
                                                           ${requestScope.ID == row.idCropType and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
