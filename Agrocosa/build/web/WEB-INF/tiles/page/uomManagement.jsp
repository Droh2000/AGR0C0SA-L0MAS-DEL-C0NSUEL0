...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            ...
            <small>Configuración de catálogo de Unidades de Medida</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" id="paramsForm" method="POST" action="uomManagement.do">
            <div class="box box-solid">
                ...
                <div class="box-body" id="info" > 
                    <c:forEach items="${UOM_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idUOM}">
                                <div class="row">
                                    
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label>Unidad de Medida</label>                                                
                                            <input name="UOMName" type="text" class="form-control pull-right" id="UOMName" required value="${row.UOMName}" />
                                        </div>                                        
                                    </div>                                     
                                ...
                                <input type="hidden" name="ID" title="${row.idUOM}" value="${row.idUOM}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.UOM_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'uomManagement.do'">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <c:forEach items="${UOM_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idUOM and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idUOM}" value="${row.idUOM}"
                                                           ${requestScope.ID == row.idUOM and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
                                            <td>${row.UOMName}</td>
                                            <td>${row.UserName}</td>
