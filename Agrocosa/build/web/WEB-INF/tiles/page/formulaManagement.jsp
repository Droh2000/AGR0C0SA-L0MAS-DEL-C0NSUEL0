...
<script type="text/javascript">
    ...
    function addFormula(idFormula)
    {

        $('#selectedIdFormula').val(idFormula);
        $('#frmAddFormula').submit();
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            ...
            <small>Configuración de Formulas</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="formulaManagement.do"  onsubmit="">
            <div class="box box-success">
                ...
                <div class="box-body" id="info" > 
                    <c:forEach items="${FormulaInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idFormula}">
                                <div class="row">


                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <label>Nombre de Formula</label>                                                
                                            <div class="input-group">
                                                <input name="FormulaName" type="text" class="form-control pull-right" id="FormulaName" required value="${row.FormulaName}" />
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                            <label></label>                                                
                                ...
                                <input type="hidden" name="ID" title="${row.idFormula}" value="${row.idFormula}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.FormulaInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'formulaManagement.do'">
                        ...
                        <div class="table-responsive">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                <thead>
                                    <tr>
                                        <th></th>                                        
                                        <th>Nombre de Formula</th>
                                        <th>Registado Por</th>
                                        <th>Fecha de Registro</th>
                                        <th>Fecha de Modificacion</th>
                                        <th>Agregar Compuestos</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <c:forEach items="${FormulaInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idFormula and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idFormula}" value="${row.idFormula}"
                                                           ${requestScope.ID == row.idFormula and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
                                            ...
                                            <td>
                                                <button 
                                                    ...
                                                    onclick="addFormula(${row.idFormula});">
                                                    <i class="fa fa-plus-square"> Compuestos</i>
                                                </button>                                                
                                            </td>
                                        </tr>
                                    </c:forEach>
        ...
        <div class="hide">
            <form name="frmAddFormula" id="frmAddFormula" method="post" action="addFormula.do">
                <input type="hidden" id="selectedIdFormula" name="idFormula" value=""/>
                <input type="hidden" name="Action" value="AddFormula"/>
            </form>
        </div>  
