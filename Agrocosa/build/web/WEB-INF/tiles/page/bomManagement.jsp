...
<script type="text/javascript">
    ...
    function addComponent(idBom, bomName)
    {
        $('#selectedIdBom').val(idBom);
        $('#selectedBomName').val(bomName);
        $('#frmAddBom').submit();
    }
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            Administración de Insumos Compuestos
            <small>Configuración de insumos compuestos</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="bomManagement.do"  onsubmit="">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Insumos Compuestos</h3>
                </div>


                <div class="box-body" id="info" > 
                    <c:forEach items="${BomInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idBom}">
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input name="BomName" type="text" class="form-control pull-right" id="BomName" required value="${row.BomName}" />
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                ...
                                <input type="hidden" name="ID" title="${row.idBom}" value="${row.idBom}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.BomInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'bomManagement.do'">
                        ...
                        <div class="table-responsive">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                <thead>
                                    <tr>
                                        <th></th>
                                        <th>Nombre de Insumo Compuesto</th>
                                        <th>Comentarios</th>
                                        <th>Registado Por</th>
                                ...
                                <tbody>
                                    <c:forEach items="${BomInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idBom and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idBom}" value="${row.idBom}"
                                                           ${requestScope.ID == row.idBom and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
                                            <td>${row.BomName}</td>
                                            <td>${row.Comments}</td>
                                            ...
                                            <td>
                                                <button 
                                                    ...
                                                    onclick="addComponent('${row.idBom}', '${row.BomName}');">
                                                    <i class="fa fa-plus-square"> Insumos</i>
                                                </button> 
                                            </td>
                                        </tr>
                                    </c:forEach>
        ...
        <div class="hide">
            <form name="frmAddBom" id="frmAddBom" method="post" action="addRawMaterialToBom.do">
                <input type="hidden" id="selectedIdBom" name="idBom" value=""/>
                <input type="hidden" id="selectedBomName" name="bomName" value=""/>
