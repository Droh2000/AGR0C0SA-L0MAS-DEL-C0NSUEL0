...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            ...
            <small>Configuración de plagas, prevención y tratamiento</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="pestsManagement.do"  onsubmit="">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Definición de plagas</h3>
                </div>


                <div class="box-body" id="info" > 
                    <c:forEach items="${PestInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idPest}">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label>Nombre de Plaga</label>                                                
                                            <div class="input-group">
                                                <input name="pestName" type="text" class="form-control pull-right" id="pestName" required value="${row.PestName}" />
                                            </div>                                  
                                        </div>
                                    </div>                                    
                                </div>
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <textarea name="description" class="form-control" rows="8" id="description">${row.Description}</textarea>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label>Prevención</label>
                                            <textarea name="prevention" class="form-control" rows="8" id="prevention">${row.Prevention}</textarea>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label>Tratamiento</label>                                                
                                            <textarea name="medication" class="form-control" rows="8" id="medication">${row.Medication}</textarea>
                                        </div>
                                    </div>                                    
                                </div>

                                <input type="hidden" name="ID" title="${row.idPest}" value="${row.idPest}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.PestInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'pestsManagement.do'">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <c:forEach items="${PestInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idPest and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idPest}" value="${row.idPest}"
                                                           ${requestScope.ID == row.idPest and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
                                            <td>${row.PestName}</td>
                                                        ...
                                                        <td>
                                                            <div>
                                                                <div class="pull-left image">
                                                                    <img style="width:50%;height:50%;" src="showPestPictureById.do?idPest=${row.idPest}" alt="plaga"/>
                                                                </div>
                                                            </div>                                                            
                                                        </td>
                                                        <td>&nbsp;</td>
                                                            ...
                                                            <c:if test="${requestScope.Method ne 'Edit'}">
                                                                <div>
                                                                    ...
                                                                    <button 
                                                                        ...
                                                                        title="Borrar Imagen"
                                                                        class="btn btn-primary" 
                                                                        onclick="confirmDeletePhoto(${row.idPest});">
        ...
        <script>
            $("[data-toggle=popover]").popover({
                html: true,
                content: function () {
                    return "<form name=\"loadImgForm\" method =\"POST\" action=\"editPestPicture.do\" enctype=\"multipart/form-data\" >"
                            + $('#popover-content').html()
                            + "<input type=\"hidden\" name=\"idPest\" id=\"idPest\" value='" + this.name + "'/>"
                            + "</form>";
                }
            });
        </script>
        ...
        <div class="hide">
            <form name="delPicForm" id="delPicForm" method="post" action="pestsManagement.do">
                <input type="hidden" id="idPestDelPic" name="idPest" value=""/>
