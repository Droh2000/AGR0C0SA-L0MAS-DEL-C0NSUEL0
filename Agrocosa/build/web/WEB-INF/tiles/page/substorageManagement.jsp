...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
             Administración de Sublocalidades de Inventario
            <small>Configuración de sublocalidades de inventario</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="substorageManagement.do"  onsubmit="">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Sublocalidades de Inventario</h3>
                </div>


                <div class="box-body" id="info" > 
                    <c:forEach items="${SubstorageInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idSubstorage}">
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            ...
                                            <div class="form-group">
                                                <select name="idStorage" id="idStorage" class="form-control" data-title="No puede estar vacio" required >
                                                    <option value=""></option>
                                                    <c:forEach items="${Storage_HashMap}" var="item">
                                                        <option value="${item.key}"  ${row.idStorage == item.key  ? 'selected' : ''}>${item.value}</option>
                                                    </c:forEach>
                                                </select>
                                            </div>  
                                        </div>
                                        <div class="form-group">
                                            <label>Nombre de Sublocalidad</label>                                                
                                            <div class="input-group">
                                                <input name="SubstorageName" type="text" class="form-control pull-right" id="SubstorageName" required value="${row.SubstorageName}" />
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                            <label></label>                                                
                                ...
                                <input type="hidden" name="ID" title="${row.idSubstorage}" value="${row.idSubstorage}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.SubstorageInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'substorageManagement.do'">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <c:forEach items="${SubstorageInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idSubstorage and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idSubstorage}" value="${row.idSubstorage}"
                                                           ${requestScope.ID == row.idSubstorage and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
