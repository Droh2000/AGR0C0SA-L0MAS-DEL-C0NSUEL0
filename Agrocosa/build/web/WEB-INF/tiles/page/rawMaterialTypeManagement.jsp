...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
             Administración de Tipos de Insumos
            <small>Configuración de tipos de insumos</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="rawMaterialTypeManagement.do"  onsubmit="">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Tipos de insumos</h3>
                </div>


                <div class="box-body" id="info" > 
                    <c:forEach items="${RawMaterialTypeInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idRawMaterialType}">
                                <div class="row">


                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <label>Nombre de Tipo de Insumo</label>                                                
                                            <div class="input-group">
                                                <input name="RawMaterialTypeName" type="text" class="form-control pull-right" id="RawMaterialTypeName" required value="${row.RawMaterialTypeName}" />
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                            <label></label>                                                
                                ...
                                <input type="hidden" name="ID" title="${row.idRawMaterialType}" value="${row.idRawMaterialType}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.RawMaterialTypeInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'rawMaterialTypeManagement.do'">
                        ...
                        <div class="table-responsive">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                <thead>
                                    <tr>
                                        <th></th>
                                        <th>Nombre de Tipo de Insumo</th>
                                        <th>Registado Por</th>
                                ...
                                <tbody>
                                    <c:forEach items="${RawMaterialTypeInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idRawMaterialType and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idRawMaterialType}" value="${row.idRawMaterialType}"
                                                           ${requestScope.ID == row.idRawMaterialType and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
