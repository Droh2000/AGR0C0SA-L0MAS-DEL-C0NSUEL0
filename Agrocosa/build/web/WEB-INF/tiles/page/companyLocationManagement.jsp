...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            Administración de Lugares en la Empresa
            <small>Configuración de catálogo de lugares</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" id="paramsForm" method="POST" action="companyLocationManagement.do">
            <div class="box box-solid">
                <div class="box-header with-border">
                    <h3 class="box-title">Ubicaciones de la Empresa</h3>
                </div>
                <div class="box-body" id="info" > 
                    <c:forEach items="${CompanyLocation_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idCompanyLocation}">
                                <div class="row">
                                    
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label>Nombre de la Ubicacion</label>                                                
                                            <input name="NameLocation" type="text" class="form-control pull-right" id="NameLocation" required value="${row.NameLocation}" />
                                        </div>                                        
                                    </div>                                     
                                ...
                                <input type="hidden" name="ID" title="${row.idCompanyLocation}" value="${row.idCompanyLocation}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                <div class="box-footer">
                    <c:if test="${requestScope.CompanyLocation_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'companyLocationManagement.do'">
                        ...
                        <c:if test="${requestScope.Method eq null or requestScope.Action eq 'Done'}"> &nbsp;
                            ...
                            <button type="button" class="btn btn-primary btnDelete" title ="Eliminar" name="Method" id="btnSubmit" value="Delete">
            ...
            <c:if test="${requestScope.CompanyLocation_Table ne null}">
                <div class="box box-solid">
                    <div class="box-header with-border">
                        <h3 class="box-title">Lugares de la Empresa</h3>
                    </div>
                    <div class="box-body" id="info" > 
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <c:forEach items="${CompanyLocation_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idCompanyLocation and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idCompanyLocation}" value="${row.idCompanyLocation}"
                                                           ${requestScope.ID == row.idCompanyLocation and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
                                            <td>${row.NameLocation}</td>
                                            <td>${row.UserName}</td>
