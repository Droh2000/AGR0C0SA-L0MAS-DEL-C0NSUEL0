...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            ...
            <small>Configuración de pallets</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="palletDefinitionManagement.do"  onsubmit="">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Definición de Pallets</h3>
                </div>


                <div class="box-body" id="info" > 
                    <c:forEach items="${PalletDefinitionInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idPalletDefinition}">
                                <div class="row">


                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <label>Nombre de Pallet</label>                                                
                                            <div class="input-group">
                                                <input name="PalletName" type="text" class="form-control pull-right" id="PalletName" required value="${row.PalletName}" />
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                            <label>Cantidad x Pallet</label>                                                
                                            <div class="input-group">
                                                <input name="Quantity" type="text" class="form-control pull-right" id="Quantity" required value="${row.Quantity}" />
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                            <label></label>                                                
                                ...
                                <input type="hidden" name="ID" title="${row.idPalletDefinition}" value="${row.idPalletDefinition}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.PalletDefinitionInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'palletDefinitionManagement.do'">
                        ...
                        <div class="table-responsive">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                <thead>
                                    <tr>
                                        <th></th>
                                        <th>Nombre de Pallet</th>
                                        <th>Cantidad x Pallet</th>
                                        <th>Registado Por</th>
                                ...
                                <tbody>
                                    <c:forEach items="${PalletDefinitionInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idPalletDefinition and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idPalletDefinition}" value="${row.idPalletDefinition}"
                                                           ${requestScope.ID == row.idPalletDefinition and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
