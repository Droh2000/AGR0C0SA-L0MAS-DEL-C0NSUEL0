 ...
 * @author Sly
 */
public class AjaxCampSectionServlet extends SIDServlet {

    String txn = "AgrocosaTransactions.CampSectionMenuByCampNameTransaction";
        ...
        try {
            ...
            if (nodeArray != null) {
                if (nodeArray.getSize() > 0
                        && nodeArray.exist("campName")
                        && !nodeArray.find("campName").isValueEmpty()) {
                    ...
                    if (txnNA != null && txnNA.exist("CampSection_HashMap")) {
                        xmlTable tab = txnNA.find("CampSection_HashMap").getTable();

                        ArrayList<String> resLst = tab.getColumnValues("SectionName");
                        if (resLst != null) {
                            if (resLst.size() > 0) {
                                resLst.remove(0);
                            }
