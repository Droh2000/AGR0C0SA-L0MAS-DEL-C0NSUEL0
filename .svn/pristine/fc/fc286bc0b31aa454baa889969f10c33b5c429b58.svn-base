 ...
 * @author Juan
 */
public class RetrieveTaskReportTransaction extends SIDWebTransaction {
     ...
     * @exception (none)
     */
    public RetrieveTaskReportTransaction() {
        super();
    ...
    public boolean Supports(xmlNodeArray nodeArray) {
        ...
        for (int i = 0; i < mandatoryTags.length; i++) {
            if (!nodeArray.existValue(mandatoryTags[i])) {
                System.out.println("<RetrieveTaskReportTransaction::Supports> " + mandatoryTags[i] + " Mandatory tag not found or value is empty/null");
        ...
        for (int i = 0; i < optionalTags.length; i++) {
            if (nodeArray.exist(optionalTags[i]) && !nodeArray.existValue(optionalTags[i])) {
                System.out.println("<RetrieveTaskReportTransaction::Supports> " + optionalTags[i] + " Optional tag not found or value is empty/null");
        ...
        for (int i = 0; i < mandatorySets.length; i++) {
            if (nodeArray.find(mandatorySets[i]) == null || nodeArray.find(mandatorySets[i]).getTable() == null || nodeArray.find(mandatorySets[i]).getTable().getRowsQty() == 0) {
                System.out.println("<RetrieveTaskReportTransaction::Supports> " + mandatorySets[i] + " Mandatory Set not found or value is empty/null");
        ...
        for (int i = 0; i < optionalSets.length; i++) {
            if (nodeArray.find(mandatorySets[i]) != null && (nodeArray.find(mandatorySets[i]).getTable() == null || nodeArray.find(mandatorySets[i]).getTable().getRowsQty() == 0)) {
                System.out.println("<RetrieveTaskReportTransaction::Supports> " + optionalSets[i] + " Optional Set not found or value is empty/null");
    ...
    public synchronized boolean PrepareStatements() {
        ...
        try {
            ...
            System.out.println("RetrieveTaskReportTransaction::PrepareStatements> SQLException: " + e.getMessage());
            return false;
        }
    }
    ...
    public synchronized xmlNodeArray Execute() throws SQLException, Exception {
        ...
        try {
            ...
            qry = "select task.idTask, "
                    ...
                    + "convert(CONCAT( "
                    + "FLOOR(HOUR(TIMEDIFF(if (task.Status = 'Terminada' or task.Status = 'Cancelada', task.ModifiedDate, now()), task.InsertDate)) / 24), ' dias,  ', "
                    + "MOD(HOUR(TIMEDIFF(if (task.Status = 'Terminada' or task.Status = 'Cancelada', task.ModifiedDate, now()), task.InsertDate)), 24), ' hrs,  ', "
                    + "MINUTE(TIMEDIFF(if (task.Status = 'Terminada' or task.Status = 'Cancelada', task.ModifiedDate, now()), task.InsertDate)), ' min') USING utf8) as ElapsedTime "
            ...
            System.out.println("RetrieveTaskReportTransaction::Execute> SQLException: " + e.getMessage());
            resultArray = new xmlNodeArray();
            ...
            System.out.println("RetrieveTaskReportTransaction::Execute> Exception: " + ex.getMessage());
            resultArray = new xmlNodeArray();
            resultArray.add("RESPONSE_CODE", "FAIL");
    ...
    public xmlNodeArray GenerateTestParameters() {
        ...
        nodeArr.add("TRANSACTION_CLASS_TO_EXECUTE", "AgrocosaTransactions.RetrieveTaskReportTransaction");
        return nodeArr;
    }
    ...
    public static void main(String[] argv) {
        try {
            RetrieveTaskReportTransaction transaction = new RetrieveTaskReportTransaction();
            SIDWebTransaction resultTransaction = null;
            xmlNodeArray inputParameterArray = null;
            //CIMDataBase database = null;
            System.out.println("Usage: java -classpath ...AgrocosaTransactions.RetrieveTaskReportTransaction");
            ...
            if (!transaction.IsValidTransaction()) {
                System.out.println(" RetrieveTaskReportTransaction contains an invalid transaction type.");
            } else {
                if (!transaction.Supports(inputParameterArray)) {
                    System.out.println(" RetrieveTaskReportTransaction does not support this list of parameters.");
                } else {
                    resultTransaction = database.ExecuteTransaction("AgrocosaTransactions.RetrieveTaskReportTransaction", inputParameterArray);
                    if (resultTransaction == null) {
                        ...
                        if (resultTransaction.GetError() != null) {
                            ...
                            if (resultTransaction.GetResultArray() == null) {
                                System.out.println("RetrieveTaskReportTransaction - No results were returned.");
                            } else {
            ...
            System.out.println("RetrieveTaskReportTransaction::main> caught exception " + e.getMessage());
            e.printStackTrace();
        }
    }
}
