...
public class SaveOutBinInventoryTransaction extends SIDWebTransaction {
     ...
     * @exception (none)
     */
    public SaveOutBinInventoryTransaction() {
        super();
    ...
    public boolean Supports(xmlNodeArray nodeArray) {
        ...
        for (int i = 0; i < mandatoryTags.length; i++) {
            if (!nodeArray.existValue(mandatoryTags[i])) {
                System.out.println("<SaveOutBinInventoryTransaction::Supports> " + mandatoryTags[i] + " Mandatory tag not found or value is empty/null");
        ...
        for (int i = 0; i < optionalTags.length; i++) {
            if (nodeArray.exist(optionalTags[i]) && !nodeArray.existValue(optionalTags[i])) {
                System.out.println("<SaveOutBinInventoryTransaction::Supports> " + optionalTags[i] + " Optional tag not found or value is empty/null");
        ...
        for (int i = 0; i < mandatorySets.length; i++) {
            if (nodeArray.find(mandatorySets[i]) == null || nodeArray.find(mandatorySets[i]).getTable() == null || nodeArray.find(mandatorySets[i]).getTable().getRowsQty() == 0) {
                System.out.println("<SaveOutBinInventoryTransaction::Supports> " + mandatorySets[i] + " Mandatory Set not found or value is empty/null");
        ...
        for (int i = 0; i < optionalSets.length; i++) {
            if (nodeArray.find(mandatorySets[i]) != null && (nodeArray.find(mandatorySets[i]).getTable() == null || nodeArray.find(mandatorySets[i]).getTable().getRowsQty() == 0)) {
                System.out.println("<SaveOutBinInventoryTransaction::Supports> " + optionalSets[i] + " Optional Set not found or value is empty/null");
    ...
    public synchronized boolean PrepareStatements() {
        ...
        try {
            ...
            System.out.println("SaveOutBinInventoryTransaction::PrepareStatements> SQLException: " + e.getMessage());
            return false;
        }
    }
    ...
    public synchronized xmlNodeArray Execute() throws SQLException, Exception {
        ...
        try {
            ...
            for (String rowsData1 : rowsData) {
                ...
                cropTypeName = fieldData[0]; //cropTypeName
                cropName = fieldData[1]; //cropName
                ...
                pstmtSelectBinInventory.setInt(4, quantity);
                rset = pstmtSelectBinInventory.executeQuery();
                if (!rset.next()) {
                    result += cropName + "|" + storageName + "|" + substorageName + "|" + quantity + ",   ";
                }
                if (rset != null) {
                    rset.close();
                    rset = null;
            ...
            for (String rowsData1 : rowsData) {
                ...
                //update
                pstmtUpdateBinInventory.setInt(1, quantity);
                ...
                if (rowsAffected > 0) {
                    ...
                    pstmtInsertBinInventoryMovement.setString(1, "");
                    pstmtInsertBinInventoryMovement.setString(2, "Salida");
                    pstmtInsertBinInventoryMovement.setString(3, cropTypeName);
                    pstmtInsertBinInventoryMovement.setString(4, cropName);
                    pstmtInsertBinInventoryMovement.setInt(5, quantity);
                    pstmtInsertBinInventoryMovement.setString(6, storageName);
                    pstmtInsertBinInventoryMovement.setString(7, substorageName);
                    pstmtInsertBinInventoryMovement.setInt(8, 0);
                    pstmtInsertBinInventoryMovement.setString(9, "Bin");
                    pstmtInsertBinInventoryMovement.setString(10, comments);
                    pstmtInsertBinInventoryMovement.setString(11, userName);
                    ...
                    if (rowsAffected > 0) {
                        ...
                        conn.commit();
                        resultArray.add("Result", "La salida de bines han sido registrados exitosamente.");
                        resultArray.add("RESPONSE_CODE", "PASS");
                        resultArray.add("RESPONSE_MESSAGE", "La salida de bines han sido registrados exitosamente.");
                        resultArray.add("RESPONSE_DETAIL", "");
                    } else {
                        conn.rollback();
                        resultArray.add("Result", "La salida de bines no pudieron ser registrados.");
                        resultArray.add("RESPONSE_CODE", "FAIL");
            ...
            conn.rollback();
            System.out.println("SaveOutBinInventoryTransaction::Execute> SQLException: " + e.getMessage());
            resultArray = new xmlNodeArray();
            resultArray.add("RESPONSE_CODE", "FAIL");
            ...
            conn.rollback();
            System.out.println("SaveOutBinInventoryTransaction::Execute> Exception: " + ex.getMessage());
            resultArray = new xmlNodeArray();
            resultArray.add("RESPONSE_CODE", "FAIL");
    ...
    public xmlNodeArray GenerateTestParameters() {
        ...
        nodeArr.add("TRANSACTION_CLASS_TO_EXECUTE", "JonesPlasticTransactions.SaveOutBinInventoryTransaction");
    ...
    public static void main(String[] argv) {
        try {
            SaveOutBinInventoryTransaction transaction = new SaveOutBinInventoryTransaction();
            SIDWebTransaction resultTransaction = null;
            xmlNodeArray inputParameterArray = null;
            //CIMDataBase database = null;
            System.out.println("Usage: java -classpath ...JonesPlasticTransactions.SaveOutBinInventoryTransaction");
            ...
            if (!transaction.IsValidTransaction()) {
                System.out.println(" SaveOutBinInventoryTransaction contains an invalid transaction type.");
            } else {
                if (!transaction.Supports(inputParameterArray)) {
                    System.out.println(" SaveOutBinInventoryTransaction does not support this list of parameters.");
                } else {
                    resultTransaction = database.ExecuteTransaction("JonesPlasticTransactions.SaveOutBinInventoryTransaction", inputParameterArray);
                    if (resultTransaction == null) {
                        ...
                        if (resultTransaction.GetError() != null) {
                            ...
                            if (resultTransaction.GetResultArray() == null) {
                                System.out.println("SaveOutBinInventoryTransaction - No results were returned.");
                            } else {
            ...
            System.out.println("SaveOutBinInventoryTransaction::main> caught exception " + e.getMessage());
            e.printStackTrace();
        }
    }
}
