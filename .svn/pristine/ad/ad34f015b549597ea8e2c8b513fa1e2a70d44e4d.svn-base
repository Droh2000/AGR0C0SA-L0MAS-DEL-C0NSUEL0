...
public class CropManagementServlet extends SIDServlet {

    String cropTypeTxn = "AgrocosaTransactions.CropTypeMenuTransaction";
    String managementTxn = "AgrocosaTransactions.CropManagementTransaction";
    ...
    protected void processRequest(HttpServletRequest request,
        ...
        try {
            ...
            cropTypeNA = executeTransaction(callTxnNA);
            ...
            if (cropTypeNA != null) {
                cropTypeNA.remove("RESPONSE_CODE");
                cropTypeNA.remove("RESPONSE_MESSAGE");
                cropTypeNA.remove("RESPONSE_DETAIL");
                txnNA.append(cropTypeNA);
            }
            
            
            if (txnNA != null) {
                txnNA.add("JspPage", "/cropManagement.page");
                session.setAttribute("TxData", txnNA);
                response.sendRedirect("management.do");
            } else {
                rd = request.getRequestDispatcher("/cropManagement.page");
