...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
             Administración de Tipos de Riego
            <small>Configuración de tipos de riego</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="irrigationTypeManagement.do"  onsubmit="">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Tipos de Riego</h3>
                </div>


                <div class="box-body" id="info" > 
                    <c:forEach items="${IrrigationTypeInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idIrrigationType}">
                                <div class="row">


                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <label>Nombre de Riego</label>                                                
                                            <div class="input-group">
                                                <input name="IrrigationTypeName" type="text" class="form-control pull-right" id="IrrigationTypeName" required value="${row.IrrigationTypeName}" />
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                            <label></label>                                                
                                ...
                                <input type="hidden" name="ID" title="${row.idIrrigationType}" value="${row.idIrrigationType}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.IrrigationTypeInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'irrigationTypeManagement.do'">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <c:forEach items="${IrrigationTypeInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idIrrigationType and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idIrrigationType}" value="${row.idIrrigationType}"
                                                           ${requestScope.ID == row.idIrrigationType and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
                                            <td>${row.IrrigationTypeName}</td>
                                            <td>${row.User}</td>
