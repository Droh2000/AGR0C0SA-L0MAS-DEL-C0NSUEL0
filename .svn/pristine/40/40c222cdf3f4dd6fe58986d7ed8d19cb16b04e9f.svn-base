 ...
 * @author Sly
 */
public class AjaxCropByCropTypeServlet extends SIDServlet {

    String txn = "AgrocosaTransactions.CropMenuByCropTypeTransaction";
        ...
        try {
            ...
            if (nodeArray != null) {
                if (nodeArray.getSize() > 0
                        && nodeArray.exist("CropType")
                        && !nodeArray.find("CropType").isValueEmpty()) {
                    ...
                    if (txnNA != null && txnNA.exist("Crop_HashMap")) {
                        xmlTable tab = txnNA.find("Crop_HashMap").getTable();
