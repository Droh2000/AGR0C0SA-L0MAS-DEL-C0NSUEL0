...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            Administración de Tipos de Cinta
            <small>Configuración de tipos de cinta de riego</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="pipeTypeManagement.do"  onsubmit="">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Tipos de Cinta de Riego</h3>
                </div>


                <div class="box-body" id="info" > 
                    <c:forEach items="${PipeTypeInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idPipeType}">
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        ...
                                        <div class="form-group">
                                            <label>Nombre de Cinta</label>                                                
                                            <input name="PipeTypeName" type="text" class="form-control pull-right" id="PipeTypeName" required value="${row.PipeTypeName}" />
                                        </div>
                                        <div class="form-group">
                                            <label>Costo por Gotero (1ph)</label>                                                
                                            <input name="DripCost" type="text" class="form-control floatNumber pull-right" id="DripCost" required value="${row.DripCost}" />
                                        </div>
                                        <div class="form-group">
                                            <label>Distancia entre Gotero (Cm)</label>                                                
                                            <input name="DripDistance" type="text" class="form-control pull-right" id="DripDistance" required value="${row.DripDistance}" />
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="form-group">
                                ...
                                <input type="hidden" name="ID" title="${row.idPipeType}" value="${row.idPipeType}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.PipeTypeInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'pipeTypeManagement.do'">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                <thead>
                                    <tr>
                                        <th></th>
                                        <th>Marca</th>
                                        <th>Nombre de Cinta</th>
                                        <th>Costo por Gotero (1Ph)</th>
                                        <th>Distancia entre Gotero (Cm)</th>
                                        <th>Registado Por</th>
                                ...
                                <tbody>
                                    <c:forEach items="${PipeTypeInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idPipeType and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idPipeType}" value="${row.idPipeType}"
                                                           ${requestScope.ID == row.idPipeType and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
                                            <td>${row.Brand}</td>
                                            <td>${row.PipeTypeName}</td>
                                            <td>${row.DripCost}</td>
                                            <td>${row.DripDistance}</td>
                                            <td>${row.User}</td>
