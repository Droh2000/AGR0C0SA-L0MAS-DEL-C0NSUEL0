...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            Administración de Productos por Pallets
            <small>Configuración de productos por pallets</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="palletProductManagement.do"  onsubmit="">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Poroductos por Pallets</h3>
                </div>


                <div class="box-body" id="info" > 
                    <c:forEach items="${PalletProductInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idPalletProduct}">
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input name="ProductName" type="text" class="form-control pull-right" id="ProductName" required value="${row.ProductName}" />
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                            <label>Tamaño</label>                                                
                                            <div class="input-group">   
                                                <input name="Size" type="text" class="form-control pull-right" id="Size" required value="${row.Size}" />
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                            <label>Color</label>                                                
                                            <div class="input-group">
                                                <input name="Color" type="text" class="form-control pull-right" id="Color" required value="${row.Color}" />
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                ...
                                <input type="hidden" name="ID" title="${row.idPalletProduct}" value="${row.idPalletProduct}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.PalletProductInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'palletProductManagement.do'">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <c:forEach items="${PalletProductInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idPalletProduct and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idPalletProduct}" value="${row.idPalletProduct}"
                                                           ${requestScope.ID == row.idPalletProduct and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
