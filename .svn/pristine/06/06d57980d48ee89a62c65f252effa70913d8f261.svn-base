 ...
 * @author Sly
 */
public class AjaxSubstorageServlet extends SIDServlet {

    String txn = "AgrocosaTransactions.SubstorageMenuByStorageNameTransaction";
        ...
        try {
            ...
            if (nodeArray != null) {
                if (nodeArray.getSize() > 0
                        && nodeArray.exist("storageName")
                        && !nodeArray.find("storageName").isValueEmpty()) {
                    ...
                    if (txnNA != null && txnNA.exist("Substorage_HashMap")) {
                        xmlTable tab = txnNA.find("Substorage_HashMap").getTable();

                        ArrayList<String> resLst = tab.getColumnValues("SubstorageName");
                        if (resLst != null) {
                            if (resLst.size() > 0) {
                                resLst.remove(0);
                            }
