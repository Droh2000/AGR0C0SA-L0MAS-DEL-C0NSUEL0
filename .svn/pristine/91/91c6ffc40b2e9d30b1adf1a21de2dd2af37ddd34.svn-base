...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
             Administración de Cuartos de Rebombeo
            <small>Configuración de Cuartos de Rebombeo</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form name="paramsForm" method="POST" action="bombManagement.do"  onsubmit="">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">Cuartos de Rebombeo</h3>
                </div>


                <div class="box-body" id="info" > 
                    <c:forEach items="${BombInfo_Table}" var="row" varStatus="status">
                        <c:choose>
                            <c:when test="${requestScope.Method == 'Edit' and requestScope.ID == row.idBomb}">
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <select name="idEngine" id="idEngine" class="form-control" data-title="No puede estar vacio" required >
                                                    <option value=""></option>
                                                    <c:forEach items="${Engine_HashMap}" var="item">
                                                        <option value="${item.key}"  ${row.EngineName == item.value  ? 'selected' : ''}>${item.value}</option>
                                                    </c:forEach>
                                                </select>                                                
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                            <label>Bomba</label>                                                
                                            <div class="input-group">
                                                <select name="idWaterPump" id="idWaterPump" class="form-control" data-title="No puede estar vacio" required >
                                                    <option value=""></option>
                                                    <c:forEach items="${WaterPump_HashMap}" var="item">
                                                        <option value="${item.key}"  ${row.WaterPumpName == item.value  ? 'selected' : ''}>${item.value}</option>
                                                    </c:forEach>
                                                </select>                                                
                                            </div>                                  
                                        </div>
                                        <div class="form-group">
                                            <label>Nombre de Cuarto de Rebombeo</label>                                                
                                            <div class="input-group">
                                                <input name="BombName" type="text" class="form-control pull-right" id="BombName" required value="${row.BombName}" />
                                            </div>                                  
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                ...
                                <input type="hidden" name="ID" title="${row.idBomb}" value="${row.idBomb}"/>
                            </c:when>
                        </c:choose>
                    </c:forEach>
                </div>
                ...
                <div class="box-footer">
                    <c:if test="${requestScope.BombInfo_Table ne null}">
                        <c:if test="${requestScope.Method ne null and requestScope.Action ne 'Done'}">&nbsp;
                            ...
                            <button type="Submit" class="btn btn-primary" title ="Cancelar" name="Method" id="btnSubmit"  onclick="location.href = 'bombManagement.do'">
                            ...
                            <table id="dtable" class="table table-bordered table-striped table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Motor</th>
                                        <th>Bomba</th>
                                        <th>Nombre de Cuarto de Rebombeo</th>
                                        <th>Registado Por</th>
                                ...
                                <tbody>
                                    <c:forEach items="${BombInfo_Table}" var="row" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${requestScope.Method ne 'Edit' or (requestScope.ID == row.idBomb and requestScope.ID ne 'new' ) }">
                                                    <input type="checkbox" class="minimal-red" name="ID" title="${row.idBomb}" value="${row.idBomb}"
                                                           ${requestScope.ID == row.idBomb and requestScope.Method != 'Save' ? 'checked="checked"' : ''} onclick="radio(this);"/>
                                                </c:if>
                                            </td>
