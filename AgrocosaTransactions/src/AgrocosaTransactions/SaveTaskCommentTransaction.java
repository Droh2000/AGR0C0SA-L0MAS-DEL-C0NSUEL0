...
public class SaveTaskCommentTransaction extends SIDWebTransaction {

    protected PreparedStatement pstmtTaskInfo;
    protected PreparedStatement pstmtInsertTaskComment;
     ...
     * @exception (none)
     */
    public SaveTaskCommentTransaction() {
        super();
    ...
    public boolean Supports(xmlNodeArray nodeArray) {
        ...
        for (int i = 0; i < mandatoryTags.length; i++) {
            if (!nodeArray.existValue(mandatoryTags[i])) {
                System.out.println("<SaveTaskCommentTransaction::Supports> " + mandatoryTags[i] + " Mandatory tag not found or value is empty/null");
        ...
        for (int i = 0; i < optionalTags.length; i++) {
            if (nodeArray.exist(optionalTags[i]) && !nodeArray.existValue(optionalTags[i])) {
                System.out.println("<SaveTaskCommentTransaction::Supports> " + optionalTags[i] + " Optional tag not found or value is empty/null");
        ...
        for (int i = 0; i < mandatorySets.length; i++) {
            if (nodeArray.find(mandatorySets[i]) == null || nodeArray.find(mandatorySets[i]).getTable() == null || nodeArray.find(mandatorySets[i]).getTable().getRowsQty() == 0) {
                System.out.println("<SaveTaskCommentTransaction::Supports> " + mandatorySets[i] + " Mandatory Set not found or value is empty/null");
        ...
        for (int i = 0; i < optionalSets.length; i++) {
            if (nodeArray.find(mandatorySets[i]) != null && (nodeArray.find(mandatorySets[i]).getTable() == null || nodeArray.find(mandatorySets[i]).getTable().getRowsQty() == 0)) {
                System.out.println("<SaveTaskCommentTransaction::Supports> " + optionalSets[i] + " Optional Set not found or value is empty/null");
    ...
    public synchronized boolean PrepareStatements() {
        ...
        try {
            ...
            pstmtInsertTaskComment = con.prepareStatement("Insert into taskcomment ( "
                    + "idTask, "
                    + "Comments, "
                    + "idUser, "
                    + "InsertDate, "
            ...
            this.addPreparedStatement(pstmtInsertTaskComment);
            ...
            System.out.println("SaveTaskCommentTransaction::PrepareStatements> SQLException: " + e.getMessage());
            return false;
        }
    }
    ...
    public synchronized xmlNodeArray Execute() throws SQLException, Exception {
        ...
        try {
            ...
            if (rset.next()) {
                ...
                taskDate = rset.getString("fTaskDate");
//                supervisorName = rset.getString("User");
            ...
            pstmtInsertTaskComment.setInt(1, idTask);
            pstmtInsertTaskComment.setString(2, comments);
            pstmtInsertTaskComment.setInt(3, idUser);
            rowsAffected = pstmtInsertTaskComment.executeUpdate();
            if (rowsAffected > 0) {
            ...
            if (boolSaveHistory) {
                ...
                if (rowsAffected > 0) {
                    ...
                    resultArray.add("RESPONSE_MESSAGE", "El comentario ha sido registrado exitosamente.");
                    resultArray.add("RESPONSE_DETAIL", "");
                } else {
                    conn.rollback();
                    resultArray.add("error", "El comentario no pudo ser registrado.");
                    resultArray.add("RESPONSE_CODE", "FAIL");
                    resultArray.add("RESPONSE_MESSAGE", "El comentario no pudo ser registrado.");
                    resultArray.add("RESPONSE_DETAIL", "");
                }
            ...
            conn.rollback();
            System.out.println("SaveTaskCommentTransaction::Execute> SQLException: " + e.getMessage());
            resultArray = new xmlNodeArray();
            resultArray.add("RESPONSE_CODE", "FAIL");
            ...
            conn.rollback();
            System.out.println("SaveTaskCommentTransaction::Execute> Exception: " + ex.getMessage());
            resultArray = new xmlNodeArray();
            resultArray.add("RESPONSE_CODE", "FAIL");
            ...
            CloseStatements();
//            System.out.println("<SaveTaskCommentTransaction::Execute> exit");
        }
    }
    ...
    public xmlNodeArray GenerateTestParameters() {
        ...
        nodeArr.add("TRANSACTION_CLASS_TO_EXECUTE", "JonesPlasticTransactions.SaveTaskCommentTransaction");
    ...
    public static void main(String[] argv) {
        try {
            SaveTaskCommentTransaction transaction = new SaveTaskCommentTransaction();
            SIDWebTransaction resultTransaction = null;
            xmlNodeArray inputParameterArray = null;
            //CIMDataBase database = null;
            System.out.println("Usage: java -classpath ...JonesPlasticTransactions.SaveTaskCommentTransaction");
            ...
            if (!transaction.IsValidTransaction()) {
                System.out.println(" SaveTaskCommentTransaction contains an invalid transaction type.");
            } else {
                if (!transaction.Supports(inputParameterArray)) {
                    System.out.println(" SaveTaskCommentTransaction does not support this list of parameters.");
                } else {
                    resultTransaction = database.ExecuteTransaction("JonesPlasticTransactions.SaveTaskCommentTransaction", inputParameterArray);
                    if (resultTransaction == null) {
                        ...
                        if (resultTransaction.GetError() != null) {
                            ...
                            if (resultTransaction.GetResultArray() == null) {
                                System.out.println("SaveTaskCommentTransaction - No results were returned.");
                            } else {
            ...
            System.out.println("SaveTaskCommentTransaction::main> caught exception " + e.getMessage());
            e.printStackTrace();
        }
    }
}
