 ...
 * @author Juan
 */
public class RetrievePickingToReceiptTransaction extends SIDWebTransaction {
     ...
     * @exception (none)
     */
    public RetrievePickingToReceiptTransaction() {
        super();
    ...
    public boolean Supports(xmlNodeArray nodeArray) {
        ...
        for (int i = 0; i < mandatoryTags.length; i++) {
            if (!nodeArray.existValue(mandatoryTags[i])) {
                System.out.println("<RetrievePickingToReceiptTransaction::Supports> " + mandatoryTags[i] + " Mandatory tag not found or value is empty/null");
        ...
        for (int i = 0; i < optionalTags.length; i++) {
            if (nodeArray.exist(optionalTags[i]) && !nodeArray.existValue(optionalTags[i])) {
                System.out.println("<RetrievePickingToReceiptTransaction::Supports> " + optionalTags[i] + " Optional tag not found or value is empty/null");
        ...
        for (int i = 0; i < mandatorySets.length; i++) {
            if (nodeArray.find(mandatorySets[i]) == null || nodeArray.find(mandatorySets[i]).getTable() == null || nodeArray.find(mandatorySets[i]).getTable().getRowsQty() == 0) {
                System.out.println("<RetrievePickingToReceiptTransaction::Supports> " + mandatorySets[i] + " Mandatory Set not found or value is empty/null");
        ...
        for (int i = 0; i < optionalSets.length; i++) {
            if (nodeArray.find(mandatorySets[i]) != null && (nodeArray.find(mandatorySets[i]).getTable() == null || nodeArray.find(mandatorySets[i]).getTable().getRowsQty() == 0)) {
                System.out.println("<RetrievePickingToReceiptTransaction::Supports> " + optionalSets[i] + " Optional Set not found or value is empty/null");
    ...
    public synchronized boolean PrepareStatements() {
        ...
        try {
            ...
            System.out.println("RetrievePickingToReceiptTransaction::PrepareStatements> SQLException: " + e.getMessage());
            return false;
        }
    }
    ...
    public synchronized xmlNodeArray Execute() throws SQLException, Exception {
        ...
        try {
            ...
            System.out.println("RetrievePickingToReceiptTransaction::Execute> SQLException: " + e.getMessage());
            resultArray = new xmlNodeArray();
            ...
            System.out.println("RetrievePickingToReceiptTransaction::Execute> Exception: " + ex.getMessage());
            resultArray = new xmlNodeArray();
            resultArray.add("RESPONSE_CODE", "FAIL");
    ...
    public xmlNodeArray GenerateTestParameters() {
        ...
        nodeArr.add("TRANSACTION_CLASS_TO_EXECUTE", "AgrocosaTransactions.RetrievePickingToReceiptTransaction");
        return nodeArr;
    }
    ...
    public static void main(String[] argv) {
        try {
            RetrievePickingToReceiptTransaction transaction = new RetrievePickingToReceiptTransaction();
            SIDWebTransaction resultTransaction = null;
            xmlNodeArray inputParameterArray = null;
            //CIMDataBase database = null;
            System.out.println("Usage: java -classpath ...AgrocosaTransactions.RetrievePickingToReceiptTransaction");
            ...
            if (!transaction.IsValidTransaction()) {
                System.out.println(" RetrievePickingToReceiptTransaction contains an invalid transaction type.");
            } else {
                if (!transaction.Supports(inputParameterArray)) {
                    System.out.println(" RetrievePickingToReceiptTransaction does not support this list of parameters.");
                } else {
                    resultTransaction = database.ExecuteTransaction("AgrocosaTransactions.RetrievePickingToReceiptTransaction", inputParameterArray);
                    if (resultTransaction == null) {
                        ...
                        if (resultTransaction.GetError() != null) {
                            ...
                            if (resultTransaction.GetResultArray() == null) {
                                System.out.println("RetrievePickingToReceiptTransaction - No results were returned.");
                            } else {
            ...
            System.out.println("RetrievePickingToReceiptTransaction::main> caught exception " + e.getMessage());
            e.printStackTrace();
        }
    }
}
