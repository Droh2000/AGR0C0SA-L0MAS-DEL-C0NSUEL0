 ...
 * @author Juan
 */
public class RetrieveTaskFormulaReportTransaction extends SIDWebTransaction {
     ...
     * @exception (none)
     */
    public RetrieveTaskFormulaReportTransaction() {
        super();
    ...
    public boolean Supports(xmlNodeArray nodeArray) {
        ...
        for (int i = 0; i < mandatoryTags.length; i++) {
            if (!nodeArray.existValue(mandatoryTags[i])) {
                System.out.println("<RetrieveTaskFormulaReportTransaction::Supports> " + mandatoryTags[i] + " Mandatory tag not found or value is empty/null");
        ...
        for (int i = 0; i < optionalTags.length; i++) {
            if (nodeArray.exist(optionalTags[i]) && !nodeArray.existValue(optionalTags[i])) {
                System.out.println("<RetrieveTaskFormulaReportTransaction::Supports> " + optionalTags[i] + " Optional tag not found or value is empty/null");
        ...
        for (int i = 0; i < mandatorySets.length; i++) {
            if (nodeArray.find(mandatorySets[i]) == null || nodeArray.find(mandatorySets[i]).getTable() == null || nodeArray.find(mandatorySets[i]).getTable().getRowsQty() == 0) {
                System.out.println("<RetrieveTaskFormulaReportTransaction::Supports> " + mandatorySets[i] + " Mandatory Set not found or value is empty/null");
        ...
        for (int i = 0; i < optionalSets.length; i++) {
            if (nodeArray.find(mandatorySets[i]) != null && (nodeArray.find(mandatorySets[i]).getTable() == null || nodeArray.find(mandatorySets[i]).getTable().getRowsQty() == 0)) {
                System.out.println("<RetrieveTaskFormulaReportTransaction::Supports> " + optionalSets[i] + " Optional Set not found or value is empty/null");
    ...
    public synchronized boolean PrepareStatements() {
        ...
        try {
            ...
            System.out.println("RetrieveTaskFormulaReportTransaction::PrepareStatements> SQLException: " + e.getMessage());
            return false;
        }
    }
    ...
    public synchronized xmlNodeArray Execute() throws SQLException, Exception {
        ...
        try {
            ...
            qry = "select task.idTask, "
                    ...
                    + "and task.Active = 1 "
                    + "and task.TaskType in ('Aplicacion Foliar','Riego') ";
            ...
            if (this.GetNodeArray().existValue("formulaName")) {
                formulaName = GetNodeArray().find("formulaName").getStringValue();
                resultArray.add("SelectedFormulaName", formulaName);
                qry += " and taskhistory.LaborTypeName like '%" + formulaName + "%' ";
            }
            ...
            System.out.println("RetrieveTaskFormulaReportTransaction::Execute> SQLException: " + e.getMessage());
            resultArray = new xmlNodeArray();
            ...
            System.out.println("RetrieveTaskFormulaReportTransaction::Execute> Exception: " + ex.getMessage());
            resultArray = new xmlNodeArray();
            resultArray.add("RESPONSE_CODE", "FAIL");
    ...
    public xmlNodeArray GenerateTestParameters() {
        ...
        nodeArr.add("TRANSACTION_CLASS_TO_EXECUTE", "AgrocosaTransactions.RetrieveTaskFormulaReportTransaction");
        return nodeArr;
    }
    ...
    public static void main(String[] argv) {
        try {
            RetrieveTaskFormulaReportTransaction transaction = new RetrieveTaskFormulaReportTransaction();
            SIDWebTransaction resultTransaction = null;
            xmlNodeArray inputParameterArray = null;
            //CIMDataBase database = null;
            System.out.println("Usage: java -classpath ...AgrocosaTransactions.RetrieveTaskFormulaReportTransaction");
            ...
            if (!transaction.IsValidTransaction()) {
                System.out.println(" RetrieveTaskFormulaReportTransaction contains an invalid transaction type.");
            } else {
                if (!transaction.Supports(inputParameterArray)) {
                    System.out.println(" RetrieveTaskFormulaReportTransaction does not support this list of parameters.");
                } else {
                    resultTransaction = database.ExecuteTransaction("AgrocosaTransactions.RetrieveTaskFormulaReportTransaction", inputParameterArray);
                    if (resultTransaction == null) {
                        ...
                        if (resultTransaction.GetError() != null) {
                            ...
                            if (resultTransaction.GetResultArray() == null) {
                                System.out.println("RetrieveTaskFormulaReportTransaction - No results were returned.");
                            } else {
            ...
            System.out.println("RetrieveTaskFormulaReportTransaction::main> caught exception " + e.getMessage());
            e.printStackTrace();
        }
    }
}
