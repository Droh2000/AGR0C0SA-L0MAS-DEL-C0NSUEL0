...
public class SaveReceiptStockTransferOrderTransaction extends SIDWebTransaction {
     ...
     * @exception (none)
     */
    public SaveReceiptStockTransferOrderTransaction() {
        super();
    ...
    public boolean Supports(xmlNodeArray nodeArray) {
        ...
        for (int i = 0; i < mandatoryTags.length; i++) {
            if (!nodeArray.existValue(mandatoryTags[i])) {
                System.out.println("<SaveReceiptStockTransferOrderTransaction::Supports> " + mandatoryTags[i] + " Mandatory tag not found or value is empty/null");
        ...
        for (int i = 0; i < optionalTags.length; i++) {
            if (nodeArray.exist(optionalTags[i]) && !nodeArray.existValue(optionalTags[i])) {
                System.out.println("<SaveReceiptStockTransferOrderTransaction::Supports> " + optionalTags[i] + " Optional tag not found or value is empty/null");
        ...
        for (int i = 0; i < mandatorySets.length; i++) {
            if (nodeArray.find(mandatorySets[i]) == null || nodeArray.find(mandatorySets[i]).getTable() == null || nodeArray.find(mandatorySets[i]).getTable().getRowsQty() == 0) {
                System.out.println("<SaveReceiptStockTransferOrderTransaction::Supports> " + mandatorySets[i] + " Mandatory Set not found or value is empty/null");
        ...
        for (int i = 0; i < optionalSets.length; i++) {
            if (nodeArray.find(mandatorySets[i]) != null && (nodeArray.find(mandatorySets[i]).getTable() == null || nodeArray.find(mandatorySets[i]).getTable().getRowsQty() == 0)) {
                System.out.println("<SaveReceiptStockTransferOrderTransaction::Supports> " + optionalSets[i] + " Optional Set not found or value is empty/null");
    ...
    public synchronized boolean PrepareStatements() {
        ...
        try {
            ...
            pstmtSelectStockTransferOrder = con.prepareStatement("SELECT "
                    ...
                    + "and stocktransferorderdetail.ProductName = ? "
                    + "Group by Proforma, TransportCompany, TruckDriverName, TruckPlate");
            ...
            System.out.println("SaveReceiptStockTransferOrderTransaction::PrepareStatements> SQLException: " + e.getMessage());
            return false;
        }
    }
    ...
    public synchronized xmlNodeArray Execute() throws SQLException, Exception {
        ...
        try {
            ...
            for (String element : div) {
                ...
                String[] qty = data[4].split("-");//ProductNAme-Quantity

                idStockTransferOrder = Integer.parseInt(key[0]);

                //select stock transfer order
                pstmtSelectStockTransferOrder.setInt(1, idStockTransferOrder);
                pstmtSelectStockTransferOrder.setString(2, key[1]);
                ...
                //get pallet id from stocktransferorderdetail
                pstmtSelectTransferOrderDetail.setInt(1, idStockTransferOrder);//idStockTransferOrder
                pstmtSelectTransferOrderDetail.setString(2, key[1]);//ProductName
                ...
                while (rset.next()) {
                    ...
                    if (rowsAffected > 0) {
                        ...
                        if (rowsAffected > 0) {
                            ...
                            pstmtInsertInventoryMovement.setString(2, "Recibo");
                            ...
                            pstmtInsertInventoryMovement.setString(12, proforma);
            ...
            if (!result.contains("0")) {
                ...
                pstmtUpdateStockTransferOrder.setString(1, "Recibida");
                pstmtUpdateStockTransferOrder.setInt(2, idUser);
                ...
                if (rowsAffected > 0) {
                    ...
                    pstmtInsertStockTransferOrderHistory.setString(3, "Recibo");
                    pstmtInsertStockTransferOrderHistory.setString(4, "Entrada");
                    ...
                    if (rowsAffected > 0) {
                        ...
                        resultArray.add("RESPONSE_MESSAGE", "Recibo registrado exitosamente.");
                        resultArray.add("RESPONSE_DETAIL", "");
                    } else {
                        conn.rollback();
                        resultArray.add("error", "El recibo no pudo ser registrado.");
                        resultArray.add("RESPONSE_CODE", "FAIL");
                        resultArray.add("RESPONSE_MESSAGE", "El recibo no pudo ser registrado.");
                        resultArray.add("RESPONSE_DETAIL", "");
                    }
                } else {
                    conn.rollback();
            ...
            conn.rollback();
            System.out.println("SaveReceiptStockTransferOrderTransaction::Execute> SQLException: " + e.getMessage());
            resultArray = new xmlNodeArray();
            resultArray.add("RESPONSE_CODE", "FAIL");
            ...
            conn.rollback();
            System.out.println("SaveReceiptStockTransferOrderTransaction::Execute> Exception: " + ex.getMessage());
            resultArray = new xmlNodeArray();
            resultArray.add("RESPONSE_CODE", "FAIL");
    ...
    public xmlNodeArray GenerateTestParameters() {
        ...
        nodeArr.add("TRANSACTION_CLASS_TO_EXECUTE", "JonesPlasticTransactions.SaveReceiptStockTransferOrderTransaction");
    ...
    public static void main(String[] argv) {
        try {
            SaveReceiptStockTransferOrderTransaction transaction = new SaveReceiptStockTransferOrderTransaction();
            SIDWebTransaction resultTransaction = null;
            xmlNodeArray inputParameterArray = null;
            //CIMDataBase database = null;
            System.out.println("Usage: java -classpath ...JonesPlasticTransactions.SaveReceiptStockTransferOrderTransaction");
            ...
            if (!transaction.IsValidTransaction()) {
                System.out.println(" SaveReceiptStockTransferOrderTransaction contains an invalid transaction type.");
            } else {
                if (!transaction.Supports(inputParameterArray)) {
                    System.out.println(" SaveReceiptStockTransferOrderTransaction does not support this list of parameters.");
                } else {
                    resultTransaction = database.ExecuteTransaction("JonesPlasticTransactions.SaveReceiptStockTransferOrderTransaction", inputParameterArray);
                    if (resultTransaction == null) {
                        ...
                        if (resultTransaction.GetError() != null) {
                            ...
                            if (resultTransaction.GetResultArray() == null) {
                                System.out.println("SaveReceiptStockTransferOrderTransaction - No results were returned.");
                            } else {
            ...
            System.out.println("SaveReceiptStockTransferOrderTransaction::main> caught exception " + e.getMessage());
            e.printStackTrace();
        }
    }
}
